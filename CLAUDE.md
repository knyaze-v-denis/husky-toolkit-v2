...
### 2026-06-27 — Главная страница и сворачиваемый сайдбар

**Главная страница (`/`):**
- Заменяет редирект на `/builder` — первое, что видит пользователь
- Сетка модулей в Notion-стиле: 2 колонки для чек-листов, 1 колонка для Builder и Audit
- Контент вертикально центрирован через `flex: 1; justify-content: center` на враппере
- Стили в `app/home.module.css`, карточки используют `composes: block`
- Лого в сайдбаре — кликабельная ссылка на `/`; раздел «Главная» в навигации убран

**Сворачиваемый сайдбар:**
- Кнопка-шеврон внизу сайдбара; состояние сохраняется в `localStorage`
- Свёрнутый режим: ширина `60px`, только иконки, лейблы скрыты
- Разделители между группами (`.sectionDivider`) — `border-top`, отступы задаются паддингами секций
- Тултип при наведении в свёрнутом режиме: `position: fixed`, появляется справа от иконки
- На мобиле кнопка сворачивания скрыта (там оверлей с `mobileOpen`)
